...
//	서버에 접속한 모든 클라이언트의 목록은 서버에 접속된 모든 클라이언트가 공유해야 하므로 static 멤버로 만든다.
	static List<PrintWriter> chatList = Collections.synchronizedList(new ArrayList<PrintWriter>());		// 멀티채팅에서 추가
	Socket socket;
	PrintWriter printWriter;
	
	public MultiChatThread() {	}
//	클라이언트가 접속될 때마다 생성되는 통신소켓을 인수로 받아 PrintWriter 클래스 객체를 생성해 서버에 접속한 모든 클라이언트의 목록을 기억하는 List 인터페이스 객체 chatList에 저장하는 생성자
	public MultiChatThread(Socket socket) {
		this.socket = socket;
		try {
//		생성자의 인수로 넘겨받은 소켓으로 클라이언트에 메시지를 전송할 때 사용하는 PrintWriter 클래스 객체를 생성한다.
...
//		대화명을 저장할 변수를 만든다. 
		...
		try {
//		접속된 모든 클라이언트가 입력하는 채팅 메세지를 읽어들일
			BufferedReader reader = new BufferedReader(new InputStreamReader(socket.getInputStream()));			// 멀티채팅에서 추가
...
//			서버에 접속된 모든 클라이언트가 입력하는 2번째 메시지부터는 채팅메시지이므로 접속을 종료할 때까지 반복하며 특정 클라이언트에서 입력한 메시지를 서버에 접속한 모든 클라이언로 전송한다.
			while (true) {
...
//				채팅창을 닫아서 메시지를 받지 못하면 무한루프를 탈출시킨다
				if(str==null) {
					break;
				}
...
//			여기로 왔다는 거는 서버에 접속한 특정 클라이언트가 채팅 창을 닫았거나 오류로 인해 채팅이 중지된 경우이므로 채팅 목록에서 제거한다.
