...
public class Server2 {
	public static void main(String[] args) {
		...
		Scanner scanCli = null;				// 클라이언트에서 전송되는 데이터를 읽는 스캐너
		Scanner scanKey = null;				// 키보드로 클라이언트로 전송할 데이터를 입력받는 스캐너
...
//		ip 주소를 key로 하고 ip를 사용하는 학생 이름을 value로 하는 TreeMap 객체를 만든다.
		...
		String[] name = {"신현식", "", "김경준", "김호석","안진석","하주영", "조영환", "김종화", "", "이석우", "선대범", "김예지", "김상희", 
				"심미현", "김동호", "", "","" ,"양대석","김찬종","조정석", "", "", "이종석", "허정원", "", "", "김민영",  "", "김혜민", "",
				"손설희", "", "김재현", ""};
...
//			통신에 사용할 나머지 객체를 선언한다.
			...
			scanCli = new Scanner(socket.getInputStream());
			scanKey = new Scanner(System.in);
			
//			socket.getInetAddress().getHostAddress(): 서버에 접속한 ip주소를 얻어온다.
//			System.out.println(socket.getInetAddress().getHostAddress()+ "님이 접속했습니다.");
			...
			printWriter.write("서버: 클라이언트 어서오고\n");
			printWriter.flush();
...
//				클라이언트에서 전송받은 메시지가 "BYE"면 통신을 끝낸다 => do ~ while을 중지한다. => while()에서 조건을 만족하지 않게한다.
