...
<!-- fetch ajax를 사용하기 위해 fetch-2.0.4.zip의 압축을 풀고 fetch.js를 WebContent/js 폴더에 추가한다. -->
...
<script type="text/javascript">
	...
	function searchProcess() {
		if (searchRequest.readyState == 4 && searchRequest.status == 200) {
			...
			var object = eval("(" + searchRequest.responseText + ")"); // 주의 요망
		...
		fetch(url, {
			...
			response.text().then(function(text) {
				...
				if (response.status == 200) {
					var table = document.getElementById("ajaxTable");
					table.innerHTML = '';
					// js ajax 요청을 받는 searchRequest.responseText 대신 fetch ajax는 response 객체에서 text()함수로 얻어낸 text를 쓴다.
					var object = eval("(" + text + ")"); // 주의 요망
					var result = object.result;
					for (var i = 0; i < result.length; i++) {
						var row = table.insertRow(0);
						for (let j = 0; j < result[i].length; j++) {
...
<!-- 회원 가입 양식 추가 -->
<div class="container">
	<table class="table" align="center" style="text-align: center; border: 1px solid black;">
		<thead>
			<tr>
		...
		<tbody>
			<tr>
				...
				<td><input id="name" size="20" type="text" class="form-control"/></td>
			</tr>
			<tr>
				<td style="background-color: #fafafa; text-align: center;"><h5>나이</h5></td>
				<td><input id="age" size="20" type="text" class="form-control"/></td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td><input id="email" size="20" type="text" class="form-control"/></td>
			</tr>

			<tr>
				<td colspan="2" > <button type="button" class="btn btn-primary pull-right" onclick="insertFunction()">입력</button></td>
			</tr>
		</tbody>
	</table>
</div>
...
<table class="table" align="center" style="text-align: center; border: 1px solid black;">
	<thead>
		...
		<tr>
			...
			<th style="background-color: LightGreen; text-align: center;">이름</th>
			<th style="background-color: LightGreen; text-align: center;">나이</th>
			<th style="background-color: LightGreen; text-align: center;">성별</th>
			<th style="background-color: LightGreen; text-align: center;">이메일</th>
		</tr>
	</thead>
	<tbody id="ajaxTable">
		<tr></tr>
