...
public class AjaxSearch extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		System.out.println("AjaxSearch 서블릿이 get방식으로 요청됨");
		actionDo(request, response);
	}
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		System.out.println("AjaxSearch 서블릿이 post방식으로 요청됨");
		actionDo(request, response);
	}
...
//		처리된 데이터가 송신할 때 컨테츠 타입을 설정한다.
		response.setContentType("text/html; charset=UTF-8");
		
		String name = request.getParameter("name");
//		System.out.println(name);
//		new AjaxDAO();
//		ajax 방식으로 요청한 곳으로 처리한 데이터를 리턴시킨다. 
//		=> ajax로 서블릿을 호출한 쪽에서 responseText를 사용해서 받는다.
...
//		얻어온 입력 문자열이 포함된 데이터들을 하나의 문자열로 연결한다.
		...
		result.append("]}");
//		StringBuffer 타입의 객체를 String 타입으로 리턴시키킥 위해 toString() 메소드를 실행해서 리턴시킨다.
