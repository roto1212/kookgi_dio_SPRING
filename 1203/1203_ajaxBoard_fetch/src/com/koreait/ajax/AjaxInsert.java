...
public class AjaxInsert extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		System.out.println("AjaxInsert 서블릿이 get방식으로 요청됨");
		actionDo(request, response);
	}

	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		System.out.println("AjaxInsert 서블릿이 post방식으로 요청됨");
		actionDo(request, response);
	}
...
//		넘겨받은 데이터를 테이블에 저장하고 처리결과(숫자)를 리턴시킨다. => write() 메소드는 인수로 문자열만 가질 수 있으므로 공백을 붙여서 문자열로 만들어 리턴시킨다.
...
//			넘겨받은 데이터가 올바르지 않으면 AjaxDAO 클래스의 테이블에 저장하는 sql명령을 실행할 수 없으므로 0을 리턴시킨다.
			return 0;
		}
